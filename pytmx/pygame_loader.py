...
def _load_images_pygame(tmxdata, mapping, *args, **kwargs):
    ...
    for ts in tmxdata.tilesets:
        ...
        # this happens if extra graphics are included on the left, but they are
        # not actually part of the tileset
    ...
    for layer in tmxdata.layers:
        if isinstance(layer, pytmx.ImageLayer):
...
def load(filename, *args, **kwargs):
    ...
    ready to use.
    """
    tmxdata = pytmx.TiledMap.parse(filename)
